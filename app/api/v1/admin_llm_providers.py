...
from app.api.v1.llm_providers import _provider_detail
...
async def get_admin_llm_provider(
    ...
    provider = await repository.get_global_llm_provider(provider_id)
    if provider is None:
        raise _not_found()
    return _provider_detail(provider)
...
async def update_admin_llm_provider(
    ...
    try:
        provider = await repository.update_global_llm_provider(
            provider_id,
            **values,
        )
